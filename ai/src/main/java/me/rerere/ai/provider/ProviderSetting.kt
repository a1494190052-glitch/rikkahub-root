...
sealed class ProviderSetting {
    ...
    data class Dsh(
        override var id: Uuid = Uuid.random(),
        override var enabled: Boolean = true,
        override var name: String = "DeepSeek Harness",
        override var models: List<Model> = emptyList(),
        override val balanceOption: BalanceOption = BalanceOption(),
        @Transient override val builtIn: Boolean = false,
        @Transient override val description: @Composable (() -> Unit) = {},
        ...
        // DSH agent preset (模式): standard / code / minimal / cordis
        var agentPreset: String = "standard",
        // DSH sandbox mode (权限): read-only / workspace-write / danger-full-access
        var sandboxMode: String = "workspace-write",
        // DSH reasoning effort: off / low / high / max
