...
/**
 * Detects busybox-ash-incompatible bash syntax so workspace_shell can switch to
 * bash only when needed (T-bash-on-demand). The rule table and its fix hints are
 * loaded from assets/bashism_rules.json (shared JSON, same file iOS loads), so
 ...
 *
 * Ported from OpenMinis (com.openminis.app.agent.shell.BashismDetector).
 */
object BashismDetector {
    ...
    fun ensureLoaded(context: Context) {
        ...
        synchronized(this) {
            if (loaded) return
            rules = try {
                val json = context.assets.open("bashism_rules.json")
