package me.rerere.rikkahub.data.ai.shell
 ...
 *   - host-side 5s network precheck before running the package manager (F2),
 *   - persistent failure backoff (24h / 3-strikes) via SharedPreferences (F2),
 *   - one install attempt per process launch.
 *
 * Adapted for RikkaHub's Ubuntu (proot) workspace: uses `apt-get` instead of
 * Alpine's `apk`. In practice the Ubuntu rootfs ships with bash pre-installed,
 * so the install path is rarely exercised; the guards still matter for the
 * offline / broken-source case.
 *
 * Ported from OpenMinis (com.openminis.app.agent.shell.OnDemandBash).
 */
object OnDemandBash {

    private const val TAG = "BashismDetector"
    ...
    private const val INSTALL_BUDGET_MS = 120_000L // apt is slower than apk
    private const val PROBE_URL = "http://archive.ubuntu.com/ubuntu/"
    ...
    suspend fun ensureBash(context: Context, executor: Executor): Outcome = lock.withLock {
        ...
        if (!networkReachable()) {
            ...
            return Outcome.Unavailable("network unreachable — cannot install bash")
        }

        Log.i(TAG, "installing bash (budget ${INSTALL_BUDGET_MS / 1000}s)…")
        val rc = executor.run("apt-get update -qq && apt-get install -y -qq bash", INSTALL_BUDGET_MS)
        ...
        if (verified) {
            ...
            // if the user later removes bash this session, one fresh reinstall
