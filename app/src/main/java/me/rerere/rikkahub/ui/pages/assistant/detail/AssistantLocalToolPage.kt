...
private fun AssistantLocalToolContent(
    ...
    Column(
        ...
        CardGroup(title = { Text("橘瓣移植工具 (OrangeChat Tools)") }) {
            item(headlineContent = { Text("Battery Info") }, supportingContent = { Text("Let the AI read device battery status, level, and charging info") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.Battery), onCheckedChange = { toggleLocalTool(LocalToolOption.Battery, it) }) })
            item(headlineContent = { Text("Brightness") }, supportingContent = { Text("Let the AI read and adjust screen brightness. Requires WRITE_SETTINGS.") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.Brightness), onCheckedChange = { toggleLocalTool(LocalToolOption.Brightness, it) }) })
            item(headlineContent = { Text("Flashlight") }, supportingContent = { Text("Let the AI turn the device flashlight on or off") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.Torch), onCheckedChange = { toggleLocalTool(LocalToolOption.Torch, it) }) })
            item(headlineContent = { Text("Vibrate") }, supportingContent = { Text("Let the AI trigger device vibration with custom duration or pattern") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.Vibrate), onCheckedChange = { toggleLocalTool(LocalToolOption.Vibrate, it) }) })
            item(headlineContent = { Text("Volume Control") }, supportingContent = { Text("Let the AI read and adjust audio volume for media/ring/notification/alarm") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.Volume), onCheckedChange = { toggleLocalTool(LocalToolOption.Volume, it) }) })
            item(headlineContent = { Text("Wake Screen") }, supportingContent = { Text("Let the AI wake up the screen when it is off") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.WakeScreen), onCheckedChange = { toggleLocalTool(LocalToolOption.WakeScreen, it) }) })
            item(headlineContent = { Text("WiFi Info") }, supportingContent = { Text("Let the AI read current WiFi connection info (SSID, signal, IP)") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.WifiInfo), onCheckedChange = { toggleLocalTool(LocalToolOption.WifiInfo, it) }) })
            item(headlineContent = { Text("Phone Info") }, supportingContent = { Text("Let the AI read SIM card and carrier information") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.TelephonyInfo), onCheckedChange = { toggleLocalTool(LocalToolOption.TelephonyInfo, it) }) })
            item(headlineContent = { Text("Storage Info") }, supportingContent = { Text("Let the AI read internal and external storage usage") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.StorageInfo), onCheckedChange = { toggleLocalTool(LocalToolOption.StorageInfo, it) }) })
            item(headlineContent = { Text("Toast") }, supportingContent = { Text("Let the AI show brief toast popup notifications on screen") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.Toast), onCheckedChange = { toggleLocalTool(LocalToolOption.Toast, it) }) })
            item(headlineContent = { Text("Post Notification") }, supportingContent = { Text("Let the AI send system notifications. Requires notification permission.") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.PostNotification), onCheckedChange = { toggleLocalTool(LocalToolOption.PostNotification, it) }) })
            item(headlineContent = { Text("Share") }, supportingContent = { Text("Let the AI open the system share sheet with text or URLs") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.Share), onCheckedChange = { toggleLocalTool(LocalToolOption.Share, it) }) })
            item(headlineContent = { Text("Media Scanner") }, supportingContent = { Text("Let the AI trigger media file scanning so files appear in gallery apps") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.ScanMedia), onCheckedChange = { toggleLocalTool(LocalToolOption.ScanMedia, it) }) })
        }
