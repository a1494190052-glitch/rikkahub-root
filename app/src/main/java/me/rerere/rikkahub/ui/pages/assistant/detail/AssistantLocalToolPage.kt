...
private fun AssistantLocalToolContent(
    ...
    Column(
        ...
        CardGroup {
            ...
            item(headlineContent = { Text("浏览器自动化 (Browser Use)") }, supportingContent = { Text("允许 AI 控制内置浏览器：打开网页、截图、点击、输入、滚动、获取 DOM 结构等") }, trailingContent = { Switch(checked = assistant.localTools.contains(LocalToolOption.BrowserUse), onCheckedChange = { toggleLocalTool(LocalToolOption.BrowserUse, it) }) })
        }
